...
import scam.lisp_objects.LString;
...
public class Parser {
	...
	private LispObject parseTail(char closedBy) throws ParseError, IOException {
		...
		return new Cons(parseSExpr(), parseTail(closedBy));
	}
	...
	/**
	 * @param token .
	 * @return True if given token is an integer.
	 */
	private boolean isIntegerNumber(String token) {
		if (token.equals("")) return false;
		// Starts with + or - is OK
		if ((token.startsWith("-") || token.startsWith("+")) && token.length() > 1) token = token.substring(1);
	...
	/**
	 * @param token .
	 * @return True if given token is a nr
	 */
	...
	/**
	 * @param token .
	 * @return True if given token is a rational number
	 */
	private boolean isRationalNumber(String token) {
		...
		return (isIntegerNumber(token.substring(0, n)) && isIntegerNumber(token.substring(n+1)));
	}
	
	/**
	 * @param token .
	 * @return True if given token is a symbol
	 */
	...
	/**
	 * @param token .
	 * @return True if given token is a quoted string
	 */
	private boolean isString(String token) {
		if ((Objects.equals("",token)) || (token.length() < 2))return false;
