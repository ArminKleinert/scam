...
public class NumberRational extends LispNumber {
	...
	public NumberReal toReal() {
		return new NumberReal(new Long(numerator).floatValue() / new Long(denominator).floatValue());
	}
	
	public String toString() {
		reduce();
		String s = new Long(numerator).toString();
		if (denominator != 1) s = s + "/" + new Long(denominator).toString();
		return s;
	}
}
