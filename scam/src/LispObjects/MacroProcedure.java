...
/**
 * MacroProcedure extends Procedure and implements a user defined function. MacroProcedure is 
 ...
 *
 */
public class MacroProcedure extends Procedure {

	private ParamTreeNode paramTreeRoot;
	private LispObject macroBody;
	 ...
	 */
	private abstract class ParamTreeNode {
	...
	private class NevalList extends ParamTreeNode { 
		protected ArrayList<ParamTreeNode> v = new ArrayList<ParamTreeNode>();
		...
		private NevalList(LispObject c) throws LispError { 
			...
			if (o != NIL.instance) {
				...
				tail = new NevalParam(o.asSymbol().getName().substring(1,o.asSymbol().getName().length()));
			}
		}
		...
		private Cons processElement(Cons c) throws LispError {
			if (c.getCar().isSymbol() && c.getCar().asSymbol().getName().equals("@")) {
				...
				v.add(new NevalParam(c.getCar().asSymbol().getName().substring(1,c.getCar().asSymbol().getName().length())));
	 ...
	 */
	private class EvalParam extends ParamTreeNode { 
	 ...
	 */
	private class NevalParam extends ParamTreeNode { 
		private String name;
		NevalParam(String s) throws LispError {
			name = s;
		}
		public LispObject process(LispObject o, Environment environment, LispObject body) throws LispError {
