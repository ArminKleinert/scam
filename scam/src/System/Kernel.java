...
public class Kernel {
	...
	public static LispObject eval(LispObject o, Environment environment) throws LispError {
		...
		if (o.isCons()) {
			LispObject pSymbol = o.asCons().objectAt(0);
			Procedure p = eval(pSymbol, environment).asProcedure();
			return apply(p, o.asCons().getCdr(), environment);
		}
	 ...
	 * @throws LispError
	 */
	public static LispObject apply(Procedure p, LispObject arguments, Environment environment) throws LispError {
		// Execute procedure
		...
		while (r.isTail()) {
			...
			LispObject pSymbol = form.objectAt(0).asSymbol();
	 ...
	 */
	public class Papply extends Procedure {
		public LispObject execute(LispObject operands, Environment environment) throws LispError {
			...
			/* Apply */
			return apply(eval(c.objectAt(0), environment).asProcedure(), eval(c.objectAt(1), environment), environment);
		}
	}
	 ...
	 * that it can be used on the language level.
	 */
	public class Peval extends Procedure {
		public LispObject execute(LispObject operands, Environment environment) throws LispError {
			/* Check input */
			Cons c = operands.asCons();
			if (c.size() != 1) throw new LispError("Wrong number of arguments");
			/* Evaluate arguments twice! */
			return eval(eval(c.objectAt(0), environment), environment);
		}
	}
}
